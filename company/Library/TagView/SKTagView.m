...
@property (nonatomic, strong) SKTagButton *seleBtn;
...
-(void)onSelected:(UIButton *)btn{
    ...
    if (btn == self.btnArray[0]) {
        if (_seleBtn.selected != YES) {
            for (SKTagButton *btn in self.btnArray) {
                if (btn == self.btnArray[0]) {
...
//                NSLog(@"取消");
        ...
        if ([self.statusArray containsObject:@"1"]){//选中
//            if (btn == self.btnArray[0]) {//第一个按钮
//                for (SKTagButton *btn in self.btnArray) {
//                    if (btn == self.btnArray[0]) {
//    
//                    }else{
...
//                }
//            }else{
//                SKTagButton *btn = self.btnArray[0];
//                btn.selected = NO;
//            }
...
//    if (![self.statusArray containsObject:@"1"] && btn == self.btnArray[0]) {
//        return;
//    }
    ...
    if (btn.selected) {
        if (btn != self.btnArray[0]) {
            ...
            //发送确定通知
//            if (!_isFirst) {
                [[NSNotificationCenter defaultCenter] postNotificationName:SHOWCERTAINNOTI object:nil];
//                _isFirst = YES;
//            }
    ...
    if ([self.statusArray containsObject:@"1"]){//选中
        if (btn == self.btnArray[0]) {//第一个按钮
            for (SKTagButton *btn in self.btnArray) {
                if (btn == self.btnArray[0]) {
                    btn.selected = YES;
//                    [[NSNotificationCenter defaultCenter] postNotificationName:HIDECERTAINNOTI object:nil];
                    //删除全部标签码
